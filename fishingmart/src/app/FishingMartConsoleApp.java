...
public class FishingMartConsoleApp {
	...
	private void menuLogin() {
	    ...
	    // 일반 회원 로그인
	    MemberVO member = ms.login(id, password);  //  MemberService에 login 메서드가 있어야 함
	    if (member != null) {
	        loggedMember = member;
	        System.out.println("로그인 " + loggedMember.getUsername() + "님 환영합니다.");
	        controlMemberMenu();
	...
	private void menuRodOrder() {
	    ...
	    if (rod == null) {
	        System.out.println("[오류] 존재하지 않는 낚시대 번호입니다.");
	        return;
	    }

	    // 3. 주문 여부 확인 (yes/no 방식)
	    String confirm = input.readString(">> 이 낚시대를 주문하시겠습니까? (yes/no): ");
	    ...
	    // 4. 주문 수량 입력
	    int quantity = input.readInt(">> 주문 수량 입력 (" + rod.getInstock() + "개 이내): ");
	    if (quantity <= 0 || quantity > rod.getInstock()) {
	        System.out.println("[오류] 재고보다 많거나 유효하지 않은 수량입니다.");
	        return;
	    }
	    ...
	    // 7. 연락처 변경 여부
	    String mobileConfirm = input.readString(">> 연락처를 변경하시겠습니까? (yes/no): ");
	    if (mobileConfirm.equals(CONFIRM)) {
	        String newMobile = input.readString(">> 새로운 연락처 입력: ");
	        order.setMobile(newMobile);
	    } else {
	        order.setMobile(loggedMember.getMobile());
	    }

	    // 8. 배송지 변경 여부
	    String addressConfirm = input.readString(">> 배송지를 변경하시겠습니까? (yes/no): ");
	    if (addressConfirm.equals(CONFIRM)) {
	        String newAddress = input.readString(">> 새로운 배송지 입력: ");
	    ...
	    if (os.orderItems(order)) {
	        System.out.println("[성공] 주문이 성공적으로 완료되었습니다.");
	        System.out.println("배송이 완료되었습니다. 감사합니다!");
	    } else {
	        System.out.println("[오류] 주문을 처리하는 데 실패했습니다.");
	    }
	}
	...
	private void menuBaitOrder() {
	    ...
	    if (bait == null) {
	        System.out.println("[오류] 존재하지 않는 미끼 번호입니다.");
	        return;
	    }

	    // 3. 주문 여부 확인
	    String confirm = input.readString(">> 이 미끼를 주문하시겠습니까? (yes/no): ");
	    if (!confirm.equals(CONFIRM)) {
	        System.out.println("주문이 취소되었습니다.");
	        return;
	    }
	    ...
	    // 4. 수량 입력
	    int quantity = input.readInt(">> 주문 수량 입력 (" + bait.getInstock() + "개 이내): ");
	    if (quantity <= 0 || quantity > bait.getInstock()) {
	        System.out.println("[오류] 재고보다 많거나 유효하지 않은 수량입니다.");
	        return;
	    }
	...
	private void menuAddItem2Cart() {
	    ...
	    int quantity = input.readInt(">> 수량 입력 (" + selectedItem.getStock() + "개 이내): ");
	...
	private void menuCartOrder() {
		...
		if (confirm.equals(CONFIRM)) {
			if (os.orderItems(order)) {
				...
				cs.clearCart();  // 장바구니 비우기 추가
				System.out.println("장바구니를 비웠습니다.");  // 안내 메시지
			} else {
